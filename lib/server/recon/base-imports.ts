...
type BaseMode = "NEW_QUARTER_BASE" | "REPLACE_QUARTER_BASE";
...
export async function executeBase(kind: BaseMode, quarterCode: string, input: SourcePayload & { previewToken:string; confirmation?:string }) { const rows=parseBase(input); const fileSha=sourceSha(input); verify(input.previewToken,kind,quarterCode,fileSha,sha(rows)); const confirmation = "替换" + quarterCode.replace("-", ""); if(kind==="REPLACE_QUARTER_BASE" && input.confirmation!==confirmation) throw invalidInput("请输入确认文本：" + confirmation);
  return withPostgresTransaction(async client=>{ await client.query("SELECT pg_advisory_xact_lock(hashtext($1))",["quarter-base:" + quarterCode]); let q=await quarter(client,quarterCode); if(!q){ if(kind!=="NEW_QUARTER_BASE") throw notFound("目标季度不存在"); const p=quarterParts(quarterCode); const day=p.quarter===1?"31":p.quarter===2?"30":p.quarter===3?"30":"31"; const cutoff=String(p.year)+"-"+String(p.quarter*3).padStart(2,"0")+"-"+day; const created=await client.query("INSERT INTO recon.quarters(code,year,quarter,cutoff_date,status) VALUES($1,$2,$3,$4,'open') RETURNING id::text",[quarterCode,p.year,p.quarter,cutoff]); q=String(created.rows[0].id); }
    const existing=await client.query("SELECT count(*)::int count FROM recon.reconciliations WHERE quarter_id=$1",[q]); if(kind==="NEW_QUARTER_BASE"&&Number(existing.rows[0].count)>0) throw conflict("目标季度已有数据"); if(kind==="REPLACE_QUARTER_BASE"){ await client.query("DELETE FROM recon.material_status WHERE quarter_id=$1",[q]); await client.query("DELETE FROM recon.reconciliations WHERE quarter_id=$1",[q]); }
...
type ReceivableTarget = { id: string; legacy_id: string; timepoint: string; account: string; region: string; customer: string; current: string };
export async function previewReceivable(quarterCode:string,payload:SourcePayload) { if(!isValidQuarterCode(quarterCode)) throw invalidInput("季度必须为 YYYY-QN"); const rows=parseUpdates(payload), fileSha=sourceSha(payload), digest=sha(rows); return withPostgresClient(async client=>{const q=await quarter(client,quarterCode);if(!q)throw notFound("目标季度不存在");const db=(await client.query("SELECT r.id::text,r.legacy_id,r.source_payload->>'timepoint' timepoint,a.name account,g.name region,c.name customer,r.company_receivable::text current FROM recon.reconciliations r JOIN recon.account_sets a ON a.id=r.account_set_id JOIN recon.customers c ON c.id=r.customer_id LEFT JOIN recon.regions g ON g.id=c.region_id WHERE r.quarter_id=$1",[q])) as {rows:ReceivableTarget[]};const bySeq=new Map(db.rows.map((r: ReceivableTarget)=>[String(r.legacy_id),r]));let matched=0,unmatched=0,conflictCount=0,unchanged=0;for(const r of rows){const x=bySeq.get(r.sequence);if(!x)unmatched++;else if(x.timepoint!==r.timepoint||x.account!==r.accountSet||x.region!==r.region||x.customer!==r.customer)conflictCount++;else if(x.current===r.companyReceivable)unchanged++;else matched++;}return {previewToken:issue("UPDATE_COMPANY_RECEIVABLE",quarterCode,fileSha,digest),quarter:quarterCode,sourceSha256:fileSha,sourceRows:rows.length,matched,unmatched,conflict:conflictCount,unchanged};}); }
...
export async function executeReceivable(
  ...
  verify(input.previewToken, "UPDATE_COMPANY_RECEIVABLE", quarterCode, fileSha, sha(rows));
