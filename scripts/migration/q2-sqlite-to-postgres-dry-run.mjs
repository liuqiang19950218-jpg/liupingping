...
function buildPlan(loaded, schema) {
  ...
  const legacySnapshots = [{ source: "q2-authoritative-sqlite", source_sha256: loaded.sqlite.sha256, captured_at: null, migration_status: "validated", preservation_plan: { file_name: loaded.sqlite.name, encoding_required: "binary-safe preservation before future PostgreSQL import", quarter: "2026-Q2" } }, { source: "q2-app-state-shared-payload", source_sha256: loaded.payload.sha256, captured_at: null, migration_status: "validated", preservation_plan: { file_name: loaded.payload.name, quarter: "2026-Q2", payload: JSON.parse(loaded.payload.raw.toString("utf8")) } }];
  ...
  const manifest = { batch_key: `q2-sqlite-postgres-dry-run:2026-Q2:${loaded.sqlite.sha256}`, quarter_code: "2026-Q2", source_sha256: loaded.sqlite.sha256, source_record_count: 851, migrated_record_count: null, status: "validated", database_connection: false, readyForDatabaseTest: false, error_details: review.filter((entry) => entry.severity === "blocking") };
