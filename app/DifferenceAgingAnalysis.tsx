    ...
    ? rows.flatMap((row) =>
        (row.differenceInvoices ?? []).flatMap((invoice, index) => {
          ...
          return [{
            ...
            differenceReason: row.cause?.trim() || invoice.category || "其他",
  ...
  return <div className="cockpit-modal aging-drawer-overlay" role="presentation" onMouseDown={onClose}>
    <section className="cockpit-detail-dialog aging-detail-dialog" role="dialog" aria-modal="true" aria-label={`${bucket.label}差额总表`} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="aging-drawer-body">
        ...
        <div className="aging-drawer-filters"><select value={accountSet} onChange={(event) => setAccountSet(event.target.value)} aria-label="账套"><option>{ALL}</option>{options("accountSet").map((value) => <option key={value}>{value}</option>)}</select><select value={region} onChange={(event) => setRegion(event.target.value)} aria-label="区域"><option>{ALL}</option>{options("region").map((value) => <option key={value}>{value}</option>)}</select><select value={owner} onChange={(event) => setOwner(event.target.value)} aria-label="负责人"><option>{ALL}</option>{options("owner").map((value) => <option key={value}>{value}</option>)}</select><input value={customer} onChange={(event) => setCustomer(event.target.value)} placeholder="搜索客户名称" aria-label="搜索客户名称" /><select value={reason} onChange={(event) => setReason(event.target.value)} aria-label="差额原因"><option>{ALL}</option>{options("differenceReason").map((value) => <option key={value}>{value}</option>)}</select><button type="button" className="aging-filter-reset" onClick={reset}>重置</button><button type="button" className="aging-filter-query" onClick={query}>查询</button></div>
