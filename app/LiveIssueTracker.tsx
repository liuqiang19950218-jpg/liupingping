...
function formatAmount(value: string) { const amount = Number(value.replace(/,/g, "")); return Number.isFinite(amount) ? amount.toLocaleString("zh-CN", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : value || "—"; }
...
export function LiveIssueTracker() {
  ...
  const cell = (column: TrackerColumn, content: ReactNode) => <td key={column.key} className={column.className} style={widthOf(column)}>{column.key === "amount" ? formatAmount(String(content ?? "")) : content}</td>;
