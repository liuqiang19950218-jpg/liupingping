...
export async function GET() {
  ...
  const sourceGitSha = process.env.SOURCE_GIT_SHA ?? null;
  if (!buildSha || !sourceGitSha || buildSha !== sourceGitSha) {
    return Response.json({ error: "VERSION_BINDING_INVALID" }, { status: 503, headers: corsHeaders });
  }
