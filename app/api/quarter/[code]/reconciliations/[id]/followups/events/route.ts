...
import { addFollowupEvent } from "../../../../../../../../lib/server/recon/write";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const event = await addFollowupEvent(code, id, body);
    return Response.json({ quarter: code, reconciliationId: id, event }, { status: 201, headers: corsHeaders });
  } catch (error) {
