...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    const items = await getDifferenceItems(id);
    return Response.json({ quarter: code, reconciliationId: id, items }, { headers: corsHeaders });
  } catch (error) {
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const item = await createDifferenceItem(code, id, body);
    return Response.json({ quarter: code, reconciliationId: id, item }, { status: 201, headers: corsHeaders });
  } catch (error) {
