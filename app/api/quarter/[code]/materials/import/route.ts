import { importQuarterMaterials } from "../../../../../../lib/server/recon/materials-import";
...
// browser XLSX flow. Server-scoped: the quarter is resolved by the server from
// [code]; the client never supplies a quarter UUID.
// Writes recon.material_status (SPD-A). Never touches spd_dashboard_rows.
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const result = await importQuarterMaterials(code, { sourceFileName, headers, rows });
