...
export function UnresolvedFollowupDashboard() {
  ...
  return <section className="uf-page" aria-busy={false}>
    <section className="uf-kpis" aria-label="未解决客户核心指标">
      {[{ label: "未解决客户总数", value: pending.length, tone: "blue" }, { label: "超期客户数", value: overdue.length, tone: "orange" }, { label: "超期金额总额", value: overdueAmount ? money(overdueAmount) : "—", tone: "red" }, { label: "财务关注事项数", value: financeItems.length, tone: "purple" }, { label: "本周新增问题数", value: weekNew, tone: "green" }].map((card) => <article className={`uf-kpi ${card.tone}`} key={card.label}><span>{card.label}</span><strong>{card.value}</strong><small>基于当前季度真实跟进数据</small></article>)}
    </section>
    ...
    <section className="uf-grid"><article className="uf-card uf-region"><header><div><p>超期客户区域跟踪</p><h2>按区域查看超期客户</h2></div><button type="button" onClick={() => setRegion(ALL)}>查看全部</button></header><div className="uf-region-body"><aside>{regionStats.map((stat) => <button key={stat.name} className={region === stat.name ? "selected" : ""} onClick={() => setRegion(stat.name)}><b>{stat.name}</b><span>{stat.list.length} 客户 · {money(stat.amount)}</span></button>)}</aside><div className="uf-region-customers"><div className="uf-region-metrics"><b>超期客户 {selectedOverdue.length}</b><b>超期金额 {money(selectedOverdue.reduce((sum, item) => sum + Math.abs(item.amount), 0))}</b><b>平均超期 {regionStats.find((stat) => stat.name === region)?.average ?? 0} 天</b></div>{selectedOverdue.length ? selectedOverdue.slice(0, 5).map((item) => <button className="uf-customer-card" key={item.id} onClick={() => setDetail(item)}><div><strong title={item.customer}>{item.customer}</strong><span>{item.region} · {item.owner || "未填写负责人"}</span></div><Badge type={riskOf(item)}>{riskOf(item)}</Badge><b>{money(item.amount)}</b><small>已超期 {dayDistance(latest(item)) ?? 0} 天 · {nextAction(item)}</small></button>) : <p className="uf-no-data">当前区域暂无超期客户。</p>}</div></div></article><article className="uf-card uf-priority"><header><div><p>财务重点关注事项</p><h2>风险优先级</h2></div></header><div className="uf-priority-tabs"><b>大额超期 TOP5</b><span>长期未更新 {priority.untouched.length}</span><span>需财务介入 {priority.finance.length}</span><span>多次无进展 {priority.stalled.length}</span></div>{priority.top.length ? priority.top.map((item, index) => <button key={item.id} onClick={() => setDetail(item)}><i>{index + 1}</i><span><strong>{item.customer}</strong><small>{item.region} · {item.owner || "未填写负责人"}</small></span><b>{money(item.amount)}</b><em>{dayDistance(latest(item)) ?? 0} 天</em></button>) : <p className="uf-no-data">暂无可识别的财务重点事项。</p>}</article></section>
    <section className="uf-grid uf-charts"><article className="uf-card"><h2>超期客户区域分布</h2>{regionStats.filter((stat) => stat.name !== ALL).map((stat) => <button className="uf-bar" key={stat.name} onClick={() => setRegion(stat.name)}><span>{stat.name}</span><i><b style={{ width: `${stat.list.length / maxCustomers * 100}%` }} /></i><em>{stat.list.length} · {(overdue.length ? stat.list.length / overdue.length * 100 : 0).toFixed(1)}%</em></button>)}</article><article className="uf-card"><h2>超期金额区域分布</h2>{regionStats.filter((stat) => stat.name !== ALL).map((stat) => <button className="uf-bar" key={stat.name} onClick={() => setRegion(stat.name)}><span>{stat.name}</span><i><b style={{ width: `${stat.amount / maxAmount * 100}%` }} /></i><em>{money(stat.amount)}</em></button>)}</article><article className="uf-card"><h2>超期账龄分布</h2><div className="uf-aging">{aging.map((bucket) => <div key={bucket.label}><b style={{ height: `${Math.max(10, bucket.value / Math.max(1, ...aging.map((item) => item.value)) * 100)}%` }} /><strong>{bucket.value}</strong><span>{bucket.label}</span></div>)}</div></article><article className="uf-card uf-advice"><h2>下一步处理建议</h2>{recommendations.length ? recommendations.map((item) => <div key={item.text}><Badge type={item.level === "high" ? "高风险" : item.level === "medium" ? "中风险" : "一般关注"}>{item.level === "high" ? "高优先级" : item.level === "medium" ? "中优先级" : "一般优先级"}</Badge><span>{item.text}</span><small>{item.action}</small></div>) : <p className="uf-no-data">当前没有需要升级处理的事项。</p>}</article></section>
