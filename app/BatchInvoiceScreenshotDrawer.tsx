...
export function BatchInvoiceScreenshotDrawer({ quarter, entries, onAppend, onClose }: { quarter: string; entries: Entry[]; onAppend: (items: Entry[]) => void; onClose: () => void }) {
  ...
  const append = () => { if (!addableRows.length) { setMessage("当前没有可填入的发票明细。"); return; } onAppend([...entries, ...addableRows.map((row) => ({ date: row.invoiceDate, invoice: row.invoiceNumber, amount: row.amount, note: "" }))]); onClose(); };
